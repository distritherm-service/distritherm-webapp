...
import { getAllCategories, getSubcategoriesByCategory, getAllBrands, getPriceRange } from '../data/products';
...
const ProductFilters: React.FC<ProductFiltersProps> = ({ filters, onFilterChange, onSaveFilters }) => {
  ...
  const [categories] = useState<string[]>(['Toutes les catégories', ...getAllCategories()]);
  ...
  useEffect(() => {
    if (filters.category !== 'Toutes les catégories') {
      const subcats = getSubcategoriesByCategory(filters.category);
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
      {/* Mobile toggle */}
      <button
        className="md:hidden w-full p-4 flex items-center justify-between text-gray-800 font-medium bg-gradient-to-r from-teal-50 to-blue-50 border-b border-gray-100"
