import axiosInstance, { saveAuthData, clearAuthData, getAccessToken, getUserData, STORAGE_KEYS } from './axiosConfig';
...
export const authService = {
  ...
  async register(data: RegisterRequest) {
    try {
      ...
      formattedPhoneNumber = formattedPhoneNumber.replace(/\s+/g, '').replace(/[^0-9]/g, '');
      ...
      const response = await axiosInstance.post('/auth/regular-register', formattedData, {
        headers: {
          'x-platform': 'web',
      ...
      console.log('Données utilisateur sauvegardées après inscription:', response.data.user);
  ...
  async loginWithGoogle(providerAuthToken: string) {
    try {
      ...
      const response = await axiosInstance.post('/auth/provider-login', payload);
  ...
  async registerWithGoogle(providerAuthToken: string, additionalInfo: any) {
    try {
      ...
      // Supprimer les espaces éventuels du numéro SIRET
      const formattedSiretNumber = additionalInfo.siretNumber.replace(/\s+/g, '');
      ...
      const response = await axiosInstance.post('/auth/provider-register', payload, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
      ...
      if (response.data.user) {
        // S'assurer que companyName et siretNumber sont inclus dans les données utilisateur 
        // même si le backend ne les retourne pas
        if (!response.data.user.companyName && formattedAdditionalInfo.companyName) {
          response.data.user.companyName = formattedAdditionalInfo.companyName;
        }
        
        if (!response.data.user.siretNumber && formattedAdditionalInfo.siretNumber) {
      ...
      console.log('Données utilisateur sauvegardées après inscription Google:', response.data.user);
  ...
  async getCurrentUserFromApi() {
    try {
      ...
      if (response.data && response.data.user) {
        ...
        console.log('Données utilisateur mises à jour depuis l\'API:', response.data.user);
  ...
  async updateProfile(userData: UpdateProfileRequest) {
    try {
      ...
      if (formattedData.siretNumber !== undefined) {
        ...
        const numericSiret = String(formattedData.siretNumber).replace(/\D/g, '');
      ...
      try {
        ...
        const response = await axiosInstance.put(`/users/${currentUser.id}`, formattedData, {
          headers: {
            'x-platform': 'web',
            'Content-Type': 'application/json'
          }
